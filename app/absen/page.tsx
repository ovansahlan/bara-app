...
import { Camera, User, Clock, CheckCircle2, RefreshCw, ChevronLeft, Calendar } from 'lucide-react';
      ...
      <div className="max-w-md mx-auto px-4 mt-5">
        ...
        <form onSubmit={handleAbsen} className="space-y-5">
          ...
          <div className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm space-y-4">
            <div>
              <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5">Nama Staf</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5">Shift</label>
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5">Tipe</label>
          ...
          {/* Area Kamera / Biometrik */}
          <div className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm">
            <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-3 text-center">Verifikasi Wajah (Selfie)</label>
            ...
            <div className="bg-zinc-50 border-2 border-dashed border-zinc-300 rounded-xl p-2 flex flex-col items-center justify-center relative overflow-hidden min-h-[260px] transition-all hover:border-indigo-400">
              {stream && !fotoBase64 && (
                <>
                  ...
                  <button type="button" onClick={jepretFoto} className="absolute bottom-4 bg-white text-indigo-600 px-6 py-2.5 rounded-full shadow-lg font-bold text-sm flex items-center gap-2 hover:bg-indigo-50 active:scale-95 transition-all">
              ...
              {!stream && !fotoBase64 && (
                <button type="button" onClick={bukaKamera} className="text-zinc-500 flex flex-col items-center gap-2 hover:text-indigo-600 transition-colors p-10">
                  <Camera size={40} strokeWidth={1.5} />
                  <span className="text-sm font-medium">Ketuk untuk buka kamera</span>
                </button>
              )}
