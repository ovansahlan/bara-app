...
export async function GET(request: Request) {
  try {
    ...
    rows.slice(1).forEach(row => {
      ...
      // TWEAK: Radar mendeteksi bulan berjalan, apapun format yang Google Sheets pakai
      const isBulanCocok = timestamp.includes(formatGlobal) || timestamp.includes(formatLokal1) || timestamp.includes(formatLokal2);

      if (isBulanCocok && nama === namaKru) {
